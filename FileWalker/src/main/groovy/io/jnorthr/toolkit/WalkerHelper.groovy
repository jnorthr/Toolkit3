...
{       
     ...
     */
	 String walkerMetadata = ".walker.path";
    ...
    */
    public WalkerHelper()
    {
    	say "\nWalkerHelper constructor()"
    	UIManager.setLookAndFeel("javax.swing.plaf.metal.MetalLookAndFeel"); 
    	priorPath = pf.getHomePath();
    	
        if ( !pf.hasPath(walkerMetadata) )
        {
        	say "... pf.hasPath(${walkerMetadata}) ? NO"
			re.returncode = JOptionPane.NO_OPTION;
        } // end of if
        else
        {
            priorPath = pf.getPath(walkerMetadata);
            say "... pf.getPath($walkerMetadata) gives priorPath of "+ priorPath
        ...
        if (re.returncode==JOptionPane.NO_OPTION) 
        {
        	...
        	if (re.chosen)
        	{
            	...
            	pf.setPath(walkerMetadata, re.path );            	
        	}
        	else
        	{
    ...
    */
    public WalkerHelper(String pathname)
    {
    	if (!(new File(pathname).exists()) || !(new File(pathname).isDirectory()) )
    	{
    		throw new java.lang.IllegalArgumentException("WalkerHelper constructor needs name of folder that exists; ${pathname} does not.")
    	} // end of if
    ...
    */
    ...
    public static void main(String[] args)
    {
        ...
        WalkerHelper wh = new WalkerHelper(); //"${homePath}Dropbox/Share");        
